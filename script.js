    ...
    fetch('/api/profile', {
        ...
        .then(data => {
            ...
            if (data.submission_history && data.submission_history.length > 0) {
                // script.js faylındakı forEach döngüsünü bununla əvəz edin
                data.submission_history.forEach(submission => {
                    const listItem = document.createElement('li');
                    // YENİ: Həm nəticəyə baxmaq, həm də yenidən iştirak etmək üçün düymələr əlavə edirik
                    listItem.innerHTML = `
        <div class="history-item-content">
            <a class="result-link" href="result.html?submission_id=${submission.id}">${submission.exam_title} - ${submission.score} bal (${submission.date})</a>
            <a href="exam-test.html?examId=${submission.exam_id}" class="retake-btn">Yenidən Başla</a>
        </div>
    `;
